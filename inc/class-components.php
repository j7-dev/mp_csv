<?php

declare(strict_types=1);

namespace MorePower\CSV;



class Components
{
	static public function renderAvatar(array $props): string
	{

		extract(\shortcode_atts(array(
			'src' => 'data:image/svg+xml;base64,',
			'title' => '',
			'size' => '6rem',
			'class' => 'text-center'
		), $props));

		$default_src = 		ob_start();
?>
		<div class="avatar <?= $class ?>">
			<div class="group inline-block rounded-full ring ring-primary ring-offset-base-100 ring-offset-2" style="width: <?= $size; ?>; height: <?= $size; ?>">
				<img class="group-hover:scale-[120%] duration-300 rounded-full object-cover" src="<?= $src ? $src : $default_src ?>" style="width: <?= $size; ?>; height: <?= $size; ?>" />
			</div>
			<?php if ($title) : ?>
				<h3 class="mt-4 font-semibold text-lg"><?= $title ?></h3>
			<?php endif; ?>
		</div>

<?php
		$html = ob_get_clean();
		return $html;
	}
}
